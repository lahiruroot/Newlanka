...
if (isset($_POST['submit'])) {
		...
		if (empty($a) and empty($b)) {
			...
			$query = "SELECT * FROM admin_db WHERE User_Name = '{$email}' AND Password = '{$hashed_password}' LIMIT 1";
					...
					$user = mysqli_fetch_assoc($result_set);
										...
										if ($user['Block_Unblock'] == 1) {
											$errors['User_Block_or_Unblock'] = 'Your Account Block';
													...
													$query = "UPDATE admin_db SET Last_Login = NOW() WHERE User_ID = '{$_SESSION['user_id']}' ";
													$result_set = mysqli_query($connection, $query);
														if (!$result_set) {
														...
														// redirect index.php
														header('Location: admin_dashboard.php?admin_login=successful login');
											}
													}  else {
