...
$user_id	= $_SESSION['user_id'];
	...
	function input($data) {
        ...
        $query = "SELECT * FROM admin_db WHERE User_ID = '{$ID}'";
						...
						$validation_msg['User_Name'] = "R";
									...
									$validation_msg['User_Name'] = "L";
											...
											$validation_msg['User_Name'] = "I";
											 ...
											 $result_set0 = mysqli_query($connection, $query);
											 $u_id = mysqli_fetch_assoc($result_set0);
						...
						$validation_msg['ID_Number'] = "R";
									...
									$validation_msg['ID_Number'] = "L";
											...
											//checking if Id number already exists
											 ...
											 $u_id = mysqli_fetch_assoc($result_set);
...
#-----------------------------------------------------------------End Form Validation-------------------------------------------------------------------------
			...
			list($a, $b, $c, $d, $e, $f, $g, $h, $i) = array_values($validation);
			...
			if (empty($a) and empty($b) and empty($c) and empty($d) and empty($e) and empty($f) and empty($g) and empty($h) and empty($i) and empty($j) and empty($k)) {
						...
						$update_query = "UPDATE admin_db SET First_Name = '{$First_Name}', Last_name = '{$Last_Name}', User_Name = '{$User_Name}', ID_Number = '{$ID_Number}', Address = '{$Address}', Telephone_Number = '{$Telephone_Number}', City = '{$City}', DOB = '{$DOB}' WHERE User_ID = '{$user_id}' ";
...
<body>
	...
	<h1 class="display-3 font-weight-bold font-italic text-center">Admin Change Details</h1>
