...
#-----------------------------------------------------------------Form Validation-----------------------------------------------------------------------------
					...
					$validation = array ("First_Name"=>"", "Last_Name"=>"", "User_Name"=>"", "ID_Number"=>"", "Password"=>"", "CPassword"=>"", "Address"=>"", "Telephone_Number"=>"", "Pro_Pic"=>"", "City"=>"", "DOB"=>"", "Gender"=>"");
					
					$lenth_check = array ("First_Name"=>"10", "Last_Name"=>"20", "User_Name"=>"20", "ID_Number"=>"10", "Password"=>"20", "CPassword"=>"20", "Address"=>"50", "Telephone_Number"=>"10", "City"=>"15");
						...
						$validation_msg['User_Name'] = "R";
									...
									$validation_msg['User_Name'] = "L";
											...
											$validation_msg['User_Name'] = "I";
											 ...
											 $query = "SELECT * FROM admin_db WHERE User_Name = '{$uname}' LIMIT 1";
											 $result_set = mysqli_query($connection, $query);
											 
											 $query2 = "SELECT * FROM admin_db WHERE User_Name = '{$uname}' LIMIT 1";
											 $result_set2 = mysqli_query($connection, $query2);
											 
											 $query3 = "SELECT * FROM admin_db WHERE User_Name = '{$uname}' LIMIT 1";
											 $result_set3 = mysqli_query($connection, $query3);
											 
											 $query4 = "SELECT * FROM admin_db WHERE User_Name = '{$uname}' LIMIT 1";
...
#-----------------------------------------------------------------End Form Validation-------------------------------------------------------------------------
			...
			if (empty($a) and empty($b) and empty($c) and empty($d) and empty($e) and empty($f) and empty($g) and empty($h) and empty($i) and empty($j) and empty($k)) {
						...
						$query = "INSERT INTO admin_db (First_Name, Last_name, User_Name, ID_Number, Password, Address, Telephone_Number, City, DOB, Gender,  Last_Login,  Block_Unblock, User_Delete) VALUES ('{$First_Name}', '{$Last_Name}', '{$User_Name}', '{$ID_Number}', '{$hashpassword}', '{$Address}', '{$Telephone_Number}', '{$City}', '{DOB}', '{}', '0000-00-00 00:00:00', '0', '0')";
						$result_set = mysqli_query($connection, $query);
						
						if ($result_set) {
							
								
						$query 		= "SELECT * FROM admin_db WHERE User_Name = '{$User_Name}'";
						$result_set = mysqli_query($connection, $query);
	...
	<div class="main">
        <div class="container-fluid">
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" enctype="multipart/form-data" id="register">
              ...
              <div class="form-group row">
                <label for="customFile" class="col-sm-2 col-form-label">Insert Profile Picture :</label>
                <div class="col-sm-6">
                  
                  <div class="custom-file">
                      <input type="file" class="custom-file-input <?php echo $validation['Pro_Pic']; ?>" id="customFile" name="pro_pic" nclick="return checkpass()">
                      <label class="custom-file-label" for="customFile">Choose file</label>
                      	<div class="invalid-feedback">
                            <?php
                            echo $validation_propic_msg['Empty_Img'];
							echo $validation_propic_msg['Check_Size'];
							echo $validation_propic_msg['Check_Type'];
							?>
                        </div>
                  </div>
                ...
                <label for="inputEmail3" class="col-sm-2 col-form-label">Gender :</label>
                    <div class="col-sm-6">
                      ...
                      <div class="custom-control custom-radio">
                          <input type="radio" id="customRadio1" name="gender" value="Male" nclick="return checkpass()" class="custom-control-input <?php echo $validation['Gender']; ?>" <?php if (isset($_POST["gender"])) {echo $male_checked;} ?>>
                          <label class="custom-control-label" for="customRadio1">Male</label>
                        </div>
                        
                        <div class="custom-control custom-radio">
                          <input type="radio" id="customRadio2" name="gender" value="Female" nclick="return checkpass()" class="custom-control-input <?php echo $validation['Gender']; ?>" <?php if (isset($_POST["gender"])) {echo $female_checked;} ?>>
