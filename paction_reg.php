...
#-----------------------------------------------------------------Form Validation-----------------------------------------------------------------------------
					...
					$lenth_check = array ("First_Name"=>"10", "Last_Name"=>"20", "User_Name"=>"100", "ID_Number"=>"10", "Password"=>"20", "CPassword"=>"20", "Address"=>"50", "Telephone_Number"=>"10", "City"=>"15");
						...
						$validation_msg['User_Name'] = "R";
									...
									$validation_msg['User_Name'] = "L";
											...
											$validation_msg['User_Name'] = "I";
											 ...
											 $query = "SELECT * FROM paction_db WHERE User_Name = '{$uname}' LIMIT 1";
...
#-----------------------------------------------------------------End Form Validation-------------------------------------------------------------------------
			...
			if (empty($a) and empty($b) and empty($c) and empty($d) and empty($e) and empty($f) and empty($g) and empty($h) and empty($i) and empty($j) and empty($k)) {
						...
						$query = "INSERT INTO paction_db (First_Name, Last_Name, User_Name, ID_Number, Password, Address, Telephone_Number, City, DOB, Last_Login,  Block_Unblock, User_Delete) VALUES ('{$First_Name}', '{$Last_Name}', '{$User_Name}', '{$ID_Number}', '{$hashpassword}', '{$Address}', '{$Telephone_Number}', '{$City}', '{DOB}', '0000-00-00 00:00:00', '0', '0')";
						$result_set = mysqli_query($connection, $query);
						
						if ($result_set) {
							
								
						$query 		= "SELECT * FROM paction_db WHERE User_Name = '{$User_Name}'";
						$result_set = mysqli_query($connection, $query);
						...
						$id = $user['ID_Number'];
								...
								header('Location: Paction_login.php');
...
<body>
	...
	<h1 class="display-3 font-weight-bold font-italic text-center">Create New Account</h1>
